...
- Se o usuário enviou um PDF de identidade visual / manual de marca, analise-o e use as cores, fontes, tom de voz e diretrizes visuais dele para personalizar o conteúdo e os prompts de imagem
...
async def generate_content(request: Request):
    try:
        ...
        brand_pdf = body.get("brand_pdf", "")        # base64 - PDF de identidade de marca
        content_pdf = body.get("content_pdf", "")     # base64 - PDF de conteúdo
        ref_images = body.get("ref_images", [])       # list of {data, media_type}

        system = SYSTEM_PROMPT
        if brand_text:
            system += f"\n\n--- IDENTIDADE DE MARCA DO USUÁRIO (TEXTO) ---\n{brand_text}\n--- FIM DA IDENTIDADE ---\nUse SOMENTE as informações acima para personalizar o conteúdo. Não invente dados adicionais."
        if brand_pdf:
            system += "\n\n[O usuário também anexou um PDF com o manual de identidade visual da marca. Analise-o para extrair cores, fontes, tom de voz e diretrizes de estilo. Use essas informações para personalizar todo o conteúdo e os prompts de imagem.]"

        # Determine if we have any files to send
        has_files = bool(content_pdf) or bool(ref_images) or bool(brand_pdf)

        # Build instruction text
        parts = []
        if brand_pdf:
            parts.append("O PDF de identidade visual da marca está anexado. Use as diretrizes visuais dele.")
        ...
        if has_files:
            ...
            # Brand PDF first (so it's seen as context)
            if brand_pdf:
                content.append({
                    "type": "document",
                    "source": {
    ...
    try:
        ...
        for attempt in range(max_retries):
            resp = requests.post(
                ...
                json={
                    "contents": [{"parts": [{"text": f"Generate a high-quality, detailed, professional image: {prompt}"}]}],
                    "generationConfig": {
